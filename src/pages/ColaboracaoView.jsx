    ...
    <div className="space-y-6 flex h-[calc(100vh-140px)] gap-6">
        ...
        <form onSubmit={handleAddTask} className="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 transition-colors duration-500">
          <div className="flex gap-3">
            <input 
              ...
              className="flex-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-4 py-3 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all shadow-sm"
            />
              ...
              <div className="flex gap-4 items-start">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-4 flex items-center gap-4">
                    ...
                    {!task.completed && (
                      <span className={`text-xs px-2 py-1 rounded-md border uppercase tracking-wider font-bold ${task.priority === 'alta' ? 'bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-800/50' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800/50'}`}>
      ...
      <div className="flex-[1] flex flex-col gap-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm transition-colors duration-500">
          ...
          <div className="text-4xl font-bold text-slate-800 dark:text-slate-100 mb-2">{tasks.filter(t => !t.completed).length}</div>
          <p className="text-sm text-slate-600 dark:text-slate-400">Mantenha seu fluxo de trabalho em dia.</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm flex-1 transition-colors duration-500">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-4">Usuários Ativos</h3>
          <div className="space-y-4">
            {['Raphael S.', 'Mariana Costa', 'Carlos Almeida', 'Roberto Silva'].map((user, i) => (
              <div key={i} className="flex items-center gap-3 group cursor-pointer">
                <div className="relative">
                  <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-xs text-slate-500 dark:text-slate-400">
                    {user.charAt(0)}
                  </div>
                ...
                <span className="text-sm font-medium text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-slate-100 transition-colors">{user}</span>
              </div>
            ))}
          </div>
        </div>
